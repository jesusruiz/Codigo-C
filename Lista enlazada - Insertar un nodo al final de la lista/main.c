...
//Incluir un nuevo nodo al final de la lista
...
void insertarAlFinal(struct Nodo** cabeza_ref, int nuevo_dato); //Función insertar un nuevo nodo al final de la lista
...
int main()
{
  ...
  */
	...
	//insertar un nuevo nodo al final la lista
...
void insertarAlInicio(struct Nodo** cabeza_ref, int nuevo_dato) {
    ...
    //*2. Establecer el dato de tipo int del nodo
    nuevo_nodo->dato = nuevo_dato;
    printf("\n\nNuevo nodo añadido: %d\n\n", nuevo_nodo->dato);

    //*3. Hacer que el puntero 'siguienteNodo' del nuevo nodo 'nuevo_nodo' apunte al que era anteriormente el nodo cabeza
    //para enlazar ambos nodos
    nuevo_nodo->siguienteNodo = (*cabeza_ref);

    //*4. pasar la 'cabeza' para que apunte al nuevo nodo
...
//Función insertarAlFinal
//Inserta un nuevo nodo al final de la lista enlazada
void insertarAlFinal(struct Nodo** cabeza_ref, int nuevo_dato) {

    //*1. Asignar nodo
    struct Nodo* nuevo_nodo = (struct Nodo*)malloc(sizeof(struct Nodo));

    struct Nodo* ultimo = *cabeza_ref;  /* utilizada en el paso 5*/

    //*2. Establecer el dato de tipo int del nodo
    nuevo_nodo->dato = nuevo_dato;
    printf("\n\nNuevo nodo añadido: %d\n\n", nuevo_nodo->dato);

    /* 3. Este nuevo nodo será el último nodo, así que el puntero 'siguienteNodo' de este nuevo nodo debe apuntar a NULL*/
    nuevo_nodo->siguienteNodo = NULL;

    //*4. Si la lista enlazada está vacía, crear el nuevo nodo como cabeza * /
    if (*cabeza_ref == NULL) {
        *cabeza_ref = nuevo_nodo;
        return;
    }

    //*5. De lo contrario, recorrer la lista hasta el último nodo
    while (ultimo->siguienteNodo != NULL) {
        ultimo = ultimo->siguienteNodo; //avanzamos al siguiente nodo
    }

    //*6. Cambiar el puntero 'siguienteNodo' del último Nodo
